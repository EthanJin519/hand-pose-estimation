...
class RenderedHandPose(Hand21KeypointDataset):
    ...
    def __getitem__(self, index):
       ...
       # heatmaps = generate_target2(keypoint2d)
        ...
        root_deep0 = keypoint3d_camera[9:10, :]
        root_deep = root_deep0[:, 2]
        keypoint3d_n = keypoint3d_camera - keypoint3d_camera[9:10, :]
        norm_scale = np.sqrt(np.sum(keypoint3d_n[0, :] ** 2))
        keypoint3d_n = keypoint3d_n / np.sqrt(np.sum(keypoint3d_n[0, :] ** 2))
        z = keypoint3d_n[:, 2]
     #   bone_length = np.sqrt(np.sum(np.square(keypoint3d_camera[9, :] - keypoint3d_camera[0, :])))
     #   keypoint3d_ex = keypoint3d_camera / np.sqrt(np.sum(np.square(keypoint3d_camera[9, :] - keypoint3d_camera[0, :])))
     #   zx = keypoint3d_ex[:, 2]
     #   root = zx[0]
     #   zx = zx - zx[0]
