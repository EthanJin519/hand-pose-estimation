...
class RegressionDisparityx1(nn.Module):
    ...
    def __init__(self, pseudo_label_generator: PseudoLabelGenerator01, criterion: nn.Module):
        ...
        self.criterion = criterion
        self.pseudo_label_generator = pseudo_label_generator
        self.pseudo_label_generator2 = PseudoLabelGenerator(21, 64, 64)

    def forward(self, y_adv, y, y_adv2, weight=None, mode='min'):
        assert mode in ['min', 'max']
        ...
        if mode == 'max':
            ...
            target = upsample(y.detach())
            b, c, _, _ = target.shape
            target = [torch.stack([target[k][j] / torch.max(target[k][j]) for j in range(c)]) for k in range(b)]
            target = torch.stack(target)
            
            ground_truth, ground_false = self.pseudo_label_generator2(y_adv2.detach())
            ground_false = ground_false + target
